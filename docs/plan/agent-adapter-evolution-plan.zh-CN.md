...
- Codex native launch resolver：**实测确认不需要**——npm wrapper 自己解析 native binary
  并 `stdio: "inherit"` 透明转发（详见 §10.4）。
...
> 实测校准（codex 0.141.0）：autoEdit flag 用 **`--sandbox workspace-write`**。旧的
> `--full-auto` 已废弃（CLI 直接告警 "use `--sandbox workspace-write` instead"）。

事件 schema（实测，每行一个 JSON）：`thread.started`(带 `thread_id`=native session)、
`turn.started`、`item.started`/`item.completed`(item.type ∈ {`agent_message`/`command_execution`/
`file_change`/`reasoning`/…})、`turn.completed`(带 usage)。注意 `--json` 只发**完整 item**，
没有 token 级增量——content 按整条 `agent_message` 落地，不是逐字流。
...
### 10.4 Codex native launch（实测结论：不需要）

实测：`codex` 是 npm wrapper（`@openai/codex/bin/codex.js`），它**自己**完成
triple→平台包→`vendor/<triple>/bin/codex` 的解析，并用 `spawn(native, argv, { stdio:
"inherit" })` + 信号转发透明地驱动 native binary。Portico 直接 spawn PATH 上的 `codex`
即可，stdin/stdout/退出码原样穿透——参考产品那套 `resolveProviderLaunch` 在这里是复刻
wrapper 的脆弱逻辑，**不做**。

唯一已知小限制：Portico 超时/取消用 `SIGKILL`，wrapper 来不及转发（SIGKILL 不可捕获），
native 子进程可能短暂成孤儿。属边缘情况，留待真实困扰时再处理（如直接 spawn native +
进程组），不在本计划范围。
