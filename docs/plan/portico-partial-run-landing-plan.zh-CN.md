...
- **P0（止血，仍需用户批准）**：给一个**不重跑 agent 就能落地好 diff** 的出口。
  - `portico apply <run_id> --allow <path>…`（或 `--override-policy`）：用户显式扩展/确认允许集后，
    对 path-policy-failed 运行执行 apply；其余前提不变（tracked tree 干净、用户批准、其它门禁仍须绿）。
    把落地留在 Portico 内，保住 clean-tree 检查与 provenance。
- **P1（判读 + 引导）**：
...
| 阶段 | 类别 | 重点 | 目标 |
| --- | --- | --- | --- |
| P0 | 流程 + 止血 | A：大任务切小 + partial 落地再续（文档）；B：`apply --allow/--override-policy` 落地 policy-failed 好 diff | 不再为一个越界文件重跑、不再绕过 apply 门禁 |
