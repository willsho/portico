...
| `--idle-timeout <ms>` | Idle watchdog timeout for this run: how long the agent may go without any sign of life — stdout/stderr, a streamed event, or (for worktree runs) a worktree file change — before it's treated as stalled. The watchdog warns first (an `idle_warning` event) and only kills at a larger hard ceiling, and widens the window during cold start / in-flight tool calls. Distinct from `--timeout` (total run length). Pass `0` or `off` to disable it (then only `--timeout` bounds the run). Omitted → the daemon's per-agent or `idleTimeoutMs` default |
