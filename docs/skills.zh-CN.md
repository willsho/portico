...
这意味着该 Skill 围绕 Portico CLI 和对工件的读取权限设计。它本身不授予广泛的 shell 访问权限。
...
然后在一个还没有生成 Skill 文件的仓库中重新运行 `portico init`，或有意识地更新已生成的文件。

由于 `init` 不会覆盖已有的 Skill 文件，团队可以在生成后定制项目级别的 Skill。
