...
4. **运行委派**并观察流式事件（工作树创建 → 代理工作 → 差异 → 测试结果）：
   ...
   ```
   有用的标志：可重复的 `--test`；可重复的 `--allowed`/`--forbidden`（路径策略）；`--base-ref <ref>`；`--cleanup manual|onNoChanges|onSuccess|always`；`--timeout <ms>`；用于机器可读事件的 `--json`。
...
5. **阅读结果，不要仅仅信任流。** 最终的 `run_done` 事件包含报告路径。阅读 `report.md`，以及结构化的 `changedFiles` 和 `tests` 对应的 `result.json`。`portico status <run_id>` 重新打印摘要（`--json` 获取结构化字段）。
...
- 连接被拒绝（Connection refused）→ 守护进程未运行：`portico start`。
