...
test("a preApply hook gates apply: a non-zero exit blocks it, a passing hook lets it through", async () => {
  installBuiltinAdapters();
  const repo = await createRepo();
  const orchestrator = createDelegationOrchestrator();
  const entry = agentEntry("codex", EDIT_AGENT);
  try {
    ...
    await writeFile(join(repo, ".portico", "config.json"), JSON.stringify({ hooks: { preApply: [{ command: "exit 3" }] } }));
    const events: DelegationEvent[] = [];
    for await (const event of orchestrator.delegate(
      { to: "codex", repo, task: "create delegated file", testCommands: ["test -f delegated.txt"] },
      { findEntry: () => entry },
    ...
    // The blocking hook stops apply, and nothing lands in the main tree.
    await assert.rejects(() => orchestrator.apply(repo, runId), /preApply hook blocked \(exit 3\)/);
    assert.equal(existsSync(join(repo, "delegated.txt")), false);
    assert.equal((await orchestrator.getRun(repo, runId)).run.status, "ready");

    // Swap in a passing hook → apply goes through.
    await writeFile(join(repo, ".portico", "config.json"), JSON.stringify({ hooks: { preApply: [{ command: "exit 0" }] } }));
    const applied = await orchestrator.apply(repo, runId);
    assert.equal(applied.run.status, "applied");
    assert.match(await readFile(join(repo, "delegated.txt"), "utf8"), /created by edit-agent/);
  } finally {
...
test("a preLaunch hook blocks a run before the agent launches", async () => {
  installBuiltinAdapters();
  const repo = await createRepo();
  const orchestrator = createDelegationOrchestrator();
  const entry = agentEntry("codex", EDIT_AGENT);
  try {
    await mkdir(join(repo, ".portico"), { recursive: true });
    await writeFile(join(repo, ".portico", "config.json"), JSON.stringify({ hooks: { preLaunch: [{ command: "exit 1" }] } }));
    const events: DelegationEvent[] = [];
    for await (const event of orchestrator.delegate(
    ...
    assert.equal(last?.type === "run_error" ? last.code : "", "hook_blocked");
    // The gate fires after the worktree is created but before the agent — so no agent_start.
    assert.equal(events.some((e) => e.type === "agent_start"), false);
  } finally {
    await rm(repo, { recursive: true, force: true });
  }
});
